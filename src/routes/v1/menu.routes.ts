...
router.post(
  ...
  auth.authenticate,
  upload.array("images"),
  (req, res) => menuController.createManyMenus(req, res),
);
router.get(
  "/menu/venues/:venueId",
  auth.authenticate,
...
router.put("/menu/:id", auth.authenticate, upload.single("image"), (req, res) =>
  menuController.updateMenu(req, res),
);
router.delete("/menu/:id", auth.authenticate, (req, res) =>
